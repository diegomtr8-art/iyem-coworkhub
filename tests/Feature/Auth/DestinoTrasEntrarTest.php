...
class DestinoTrasEntrarTest extends TestCase
{
    ...
    public function test_el_equipo_no_acaba_en_el_portal_por_un_destino_previsto(): void
    {
        ...
        // Un destino cualquiera del portal del miembro; sirve para comprobar que
        // el equipo no aterriza ahí. (`mis-facturas` se retiró al unificar los
        // pagos en «Mis accesos»; ver el comentario 2.8 en routes/web.php.)
        $this->get(route('portal.reservas', absolute: false))->assertRedirect(route('login', absolute: false));
    ...
    public function test_el_destino_previsto_de_su_propio_portal_se_respeta(): void
    {
        ...
        $destino = route('portal.reservas', absolute: false);

        $this->get($destino)->assertRedirect(route('login', absolute: false));
